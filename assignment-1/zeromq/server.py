...
print(
    f"[x] Server {server_id} listening on localhost:{port}")
articles = []
clientele = set()
...
while True:
    ...
    if method == 'JoinServer':
        ...
        result = filter(lambda x: x['type'] == params['type'],
                        result) if 'type' in params else result
        result = filter(lambda x: x['author'] == params['author'],
                        result) if 'author' in params else result
