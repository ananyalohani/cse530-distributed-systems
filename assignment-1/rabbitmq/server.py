...
class Server(object):
    ...
    def on_response(self, ch, method, props, body):
        if self.correlation_id == props.correlation_id:
            ...
            try:
                ...
                self.articles = list(
                    {v['time']: v for v in self.articles}.values())
                # print(self.articles)
                self.send_articles()
                self.response = None
    ...
    def start(self):
        ...
        if a1 == 'yes':
            q2 = inquirer.Text(
                name='parent_id', message='Enter the parent server id')
    ...
    def on_request(self, ch, method, props, body):
        ...
        if payload['method'] == 'JoinServer':
            ...
            ch.basic_publish(exchange='', routing_key=props.reply_to,
                             properties=pika.BasicProperties(correlation_id=props.correlation_id), body=json.dumps(self.articles))
            ch.basic_ack(delivery_tag=method.delivery_tag)
